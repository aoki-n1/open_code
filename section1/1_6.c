// A program to calculate the quotient and remainder of n ÷ m
// n ÷ m の商と余りを計算するプログラム
...
int main (void){
    ...
    /* calculation quotient and remainder by n ÷ m : n ÷ m の商と余りを計算 */
    // For negative integer n and positive integer m, there is a difference in the handling of remainders
    // 負整数 n と正整数 m の場合, 余りの扱いに差異あり
    if ((n < 0) && (m > 0)) printf("%ld/%ld = %d .. %ld\n", n, m, ((int)n/(int)m - 1), n-(((int)n/(int)m) - 1)*m);
    // For negative integer n and m, there is a difference in the handling of remainders
    // 負整数 n と負整数 m の場合, 余りの扱いに差異あり
    else if((n < 0) && (m < 0)) printf("%ld/%ld = %d .. %ld\n", n, m, ((int)n/(int)m + 1), n-(((int)n/(int)m) + 1)*m);
    else printf("%ld/%ld = %d .. %ld\n", n, m, (int)n/(int)m, n-(((int)n/(int)m))*m);
